...
export default function LocationSearch() {
  ...
  function handleGeolocation() {
    navigator.geolocation.getCurrentPosition(async (position) => {
      ...
      try {
        const { data } = await axios.get<Address>(
          `https://api.locationiq.com/v1/reverse?key=${env.VITE_LOCATIONIQ_API_KEY}&lat=${latitude}&lon=${longitude}&format=json&countrycodes=IT&normalizecity=1`,
        );
    ...
    <Box>
      {openEditLocationDialog && (
        <EditLocationDialog
          ...
          onCloseDialogs={() => setOpenEditLocationDialog(false)}
        />
      )}
